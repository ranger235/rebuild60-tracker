...
import { migrateSingletonArtifact, type ArtifactType } from "./lib/artifacts";
...
useEffect(() => {
  ...
  useEffect(() => {
    const singletonArtifacts: { key: string; type: ArtifactType }[] = [
      { key: "program_state_v1", type: "program_state" },
      { key: "active_block_plan_v1", type: "block_plan" },
      { key: "friction_profile_v1", type: "friction_profile" },
      { key: "next_session_priority_v1", type: "next_session_priority" }
    ];

    for (const item of singletonArtifacts) {
      try {
        migrateSingletonArtifact(item.key, item.type);
      } catch (err) {
        console.warn(`[artifacts] Failed to migrate ${item.key}`, err);
      }
    }
  }, []);
