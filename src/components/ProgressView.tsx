...
function bannerStyle(kind: "info" | "warn") {
  return {
    ...
    background:
      kind === "warn" ? "rgba(255,180,0,0.08)" : "rgba(255,255,255,0.06)",
    padding: 10,
    borderRadius: 12,
  } as const;
}
...
export default function ProgressView({
  ...
  // Settings
  const [checkinDow, setCheckinDowState] = useState<number>(() =>
    getCheckinDow(userId),
  );
  ...
  const [compareView, setCompareView] = useState<"slider" | "ghost" | "map">(
    "slider",
  );
  ...
  const [flipView, setFlipView] = useState<"normal" | "ghost" | "diff" | "map">(
    "normal",
  );
  ...
  const weekWindow = useMemo(
    () => getWeekWindowForDate(dayDate, checkinDow),
    [dayDate, checkinDow],
  );
    ...
    (async () => {
      try {
        await drawOverlayCanvas(
          ...
          curUrl,
          {
            x: (prev.align_x ?? 0) as number,
            y: (prev.align_y ?? 0) as number,
          },
  ...
  useEffect(() => {
    (async () => {
      ...
      if (daily) {
        if ((daily.weight_lbs ?? null) != null)
          setWeightLbs(String(daily.weight_lbs));
        if ((daily.waist_in ?? null) != null)
          setWaistIn(String(daily.waist_in));
        if ((daily.notes ?? "").trim() && !notes.trim())
  ...
  const weekPoseRows = useMemo(() => {
    const byPose: Record<Pose, ProgressPhotoRow[]> = {
      front: [],
      quarter: [],
      side: [],
      back: [],
    ...
    for (const r of rows) {
      if (!inRange(r.taken_on, weekWindow.weekStart, weekWindow.weekEnd))
        continue;
  ...
  const anchorsByPose = useMemo(() => {
    const by: Record<Pose, ProgressPhotoRow[]> = {
      front: [],
      quarter: [],
      side: [],
      back: [],
  ...
  const monthlyHighlights = useMemo(() => {
    ...
    const highlights: Record<
      Pose,
      { first?: ProgressPhotoRow; last?: ProgressPhotoRow }
    > = {
      front: {},
      side: {},
      back: {},
  ...
  const [visionFocus, setVisionFocus] = useState<
    "balanced" | "lower" | "upper"
  >("balanced");
  ...
  async function buildInsightPayload() {
    ...
    for (const p of CORE_POSES) {
      const h = (monthlyHighlights.highlights as any)[p] as {
        first?: ProgressPhotoRow;
        last?: ProgressPhotoRow;
      };
      if (h?.first)
        await addImg(`${p.toUpperCase()} FIRST (${h.first.taken_on})`, h.first);
      if (h?.last && h.last.id !== h.first?.id)
        await addImg(`${p.toUpperCase()} LAST (${h.last.taken_on})`, h.last);
    }
    ...
    try {
      ...
      if (nextText) {
        setAiInsightHistory((prev) =>
          [{ id, ts, text: nextText }, ...prev].slice(0, 12),
        );
      }
  ...
  async function runVisionPhysiqueAnalysis() {
    ...
    if (!a || !b) {
      alert(
        "Need at least two anchor photos for that pose (or two anchors in the current month).",
      );
      return;
    }
    ...
    try {
      ...
      const { data: sa, error: sea } = await supabase.storage
        .from("progress-photos")
        .createSignedUrl(a.storage_path, 60 * 60);
      if (sea) throw sea;
      const { data: sb, error: seb } = await supabase.storage
        .from("progress-photos")
      ...
      if (!imageA || !imageB)
        throw new Error("Could not load signed photo URLs.");
      ...
      setVisionHistory((prev) =>
        [{ id, ts, pose, scope: visionScope, text: nextText }, ...prev].slice(
          0,
          24,
        ),
      );
  ...
  function updateLocalAlign(photoId: string, x: number, y: number) {
    setRows((prev) =>
      prev.map((r) =>
        r.id === photoId ? { ...r, align_x: x, align_y: y } : r,
      ),
    );
  }
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (CORE_POSES.includes(pose)) {
        const { weekStart, weekEnd } = getWeekWindowForDate(
          dayDate,
          checkinDow,
        );
        ...
        const already = rows.some(
          (r) =>
            r.pose === pose &&
            inRange(r.taken_on, weekStart, weekEnd) &&
            (r.is_anchor ?? false),
        );
      ...
      if (checkinMode) {
        const nextMissing = CORE_POSES.find((p) => {
          ...
          return !rows.some(
            (r) =>
              r.pose === p &&
              inRange(r.taken_on, weekWindow.weekStart, weekWindow.weekEnd),
          );
        });
        if (nextMissing) {
          ...
          alert(
            "Weekly check-in complete. Front/Side/Back are all in the bag.",
          );
        }
      }
    } catch (e: any) {
  ...
  async function openCompareForRow(r: ProgressPhotoRow) {
    ...
    const prev = (anchorsByPose[r.pose] ?? []).find(
      (x) => x.taken_on < r.taken_on,
    );
    if (!prev) {
      alert(
        "No previous anchor found for this pose yet. Upload at least 2 weekly anchors.",
      );
      return;
    }
    try {
  ...
  if (!userId) {
    async function drawOverlayCanvas(
      ...
      const w = 320;
      const h = Math.round(
        (w * imgB.naturalHeight) / Math.max(1, imgB.naturalWidth),
      );
      canvas.width = w;
      canvas.height = h;
      const ctx = canvas.getContext("2d");
      ...
      const makeOutline = (
        ...
        for (let y = 1; y < h - 1; y++) {
          for (let x = 1; x < w - 1; x++) {
            ...
            const down =
              (px[i + w * 4] + px[i + w * 4 + 1] + px[i + w * 4 + 2]) / 3;
    ...
    useEffect(() => {
      if (!compareOpen || compareView !== "map" || !compareA || !compareB)
        return;
      ...
      (async () => {
        try {
          await drawOverlayCanvas(
            ...
            curUrl,
            {
              x: (compareA.align_x ?? 0) as number,
              y: (compareA.align_y ?? 0) as number,
            },
            {
              x: (compareB.align_x ?? 0) as number,
              y: (compareB.align_y ?? 0) as number,
            },
            compareOpacity,
          );
        } catch {}
    ...
    <div>
      ...
      >
        ...
        <span style={{ opacity: 0.75 }}>
          Week window: <strong>{weekWindow.weekStart}</strong> →{" "}
          <strong>{weekWindow.weekEnd}</strong>
        </span>
      </div>
      ...
      {mode === "photos" && (
        <div>
          ...
          >
            <div
              style={{
                ...bannerStyle(weekComplete ? "info" : "warn"),
                marginBottom: 12,
              }}
            >
              <div
                ...
                <div>
                  ...
                  {!weekComplete ? (
                    <div style={{ marginTop: 8, opacity: 0.9 }}>
                      Upload all 3 poses to unlock the rest of Progress:{" "}
                      <strong>Compare</strong>, <strong>Flipbook</strong>, and{" "}
                      <strong>Monthly highlights</strong>.
                    </div>
                  ) : (
                    <div style={{ marginTop: 8, opacity: 0.9 }}>
                      ✅ Week complete. Check-in is ready to feed the rest of
                      Progress.
                    </div>
                  )}
                </div>
                  ...
                  {!checkinMode ? (
                    ...
                    <button
                      onClick={() => {
                        setCheckinMode(false);
                        alert(
                          "Check-in cancelled (nothing deleted). You can resume anytime.",
                        );
                      }}
                    >
                      Cancel Check-In
              ...
              {isThisWeek && !weekComplete && !promptDismissed ? (
                ...
                >
                  <div style={{ opacity: 0.95 }}>
                    Missing this week:{" "}
                    <strong>
                      {weekMissing.map((p) => p.toUpperCase()).join(" / ")}
                    </strong>
                    . One clean check-in per week, and you’ve got a movie by
                    summer.
                  </div>
                  <button
                    onClick={() => {
            ...
            {checkinMode ? (
              <div style={{ ...bannerStyle("info"), marginBottom: 12 }}>
                <strong>Check-In Mode:</strong> Upload{" "}
                <strong>{checkinStep.toUpperCase()}</strong> next.
                <span style={{ opacity: 0.8 }}>
                  {" "}
                  (After upload, it will auto-advance.)
                </span>
              </div>
            ) : null}
            ...
            <div style={{ display: "grid", gap: 8, maxWidth: 560 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <button
                  onClick={async () => {
                    const daily = await localdb.dailyMetrics.get([
                      userId,
                      dayDate,
                    ]);
                    if (daily?.weight_lbs != null)
                      setWeightLbs(String(daily.weight_lbs));
                    if (daily?.waist_in != null)
                      setWaistIn(String(daily.waist_in));
                    if ((daily?.notes ?? "").trim())
                      setNotes(daily?.notes ?? "");
          ...
          >
            ...
            >
              <div style={{ display: "grid", gap: 12 }}>
                ...
                >
                  <div>
                    <strong>Monthly Scorecard + Coach Analysis</strong>{" "}
                    <span style={{ opacity: 0.8 }}>
                      ({monthStats.monthKey})
                    </span>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      <div style={{ marginTop: 4, opacity: 0.8, fontSize: 12 }}>
                        Manual analysis actions. Keep these when you want a
                        fresh structured score, a new coach read, or a vision
                        pass.
                      </div>
                    </div>
                    ...
                    >
                      <button
                        onClick={generatePhysiqueScorecard}
                        disabled={scoreBusy}
                        title="Generate a 1–10 monthly scorecard"
                      >
                        {scoreBusy ? "Scoring…" : "Run Scorecard"}
                      </button>
                      <button
                        onClick={generateAiPhysiqueInsight}
                        disabled={aiBusy}
                      >
                        {aiBusy ? "Generating AI…" : "Run AI"}
                      </button>
                      <button
                        onClick={runVisionPhysiqueAnalysis}
                        disabled={visionBusy}
                        title="Compare two photos with Vision AI"
                      >
                      ...
                      <button
                        ...
                        disabled={!aiInsight}
                        title="Clear the current AI output"
                      >
                        Clear AI
                      </button>
                      <button
                        onClick={() => setVisionText("")}
                        disabled={!visionText}
                        title="Clear the current Vision output"
                      >
                        Clear vision
                      </button>
                    </div>
                  ...
                  >
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={visionAppendMode}
                        onChange={(e) => setVisionAppendMode(e.target.checked)}
                      />
                      Append Vision runs
                    </label>
                    <label style={{ fontSize: 12, opacity: 0.9 }}>
                      ...
                      <select
                        value={visionPose}
                        onChange={(e) => setVisionPose(e.target.value as Pose)}
                        style={{ padding: 6 }}
                      >
                        <option value="front">Front</option>
                        <option value="quarter">Quarter Turn</option>
                    ...
                    <label style={{ fontSize: 12, opacity: 0.9 }}>
                      ...
                      <select
                        value={visionScope}
                        onChange={(e) => setVisionScope(e.target.value as any)}
                        style={{ padding: 6 }}
                      >
                    ...
                    <label style={{ fontSize: 12, opacity: 0.9 }}>
                      ...
                      <select
                        value={visionFocus}
                        onChange={(e) => setVisionFocus(e.target.value as any)}
                        style={{ padding: 6 }}
                      >
              ...
              <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ marginTop: 6, opacity: 0.8, fontSize: 12 }}>
                      ...
                      {monthStats.avgSleep == null
                        ? "—"
                        : monthStats.avgSleep.toFixed(1)}
                      h • Avg protein:{" "}
                      {monthStats.avgProtein == null
                        ? "—"
                        : Math.round(monthStats.avgProtein)}
                      g • Avg Zone2:{" "}
                      {monthStats.avgZone2 == null
                        ? "—"
                        : Math.round(monthStats.avgZone2)}
                      m
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div style={{ marginTop: 6, opacity: 0.8, fontSize: 12 }}>
                      Tip: Quick Log is your “daily signal.” Measurements are
                      your “official tape.”
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div style={{ marginTop: 6, opacity: 0.85, fontSize: 12 }}>
                      1–10 ratings for this month. Use it to see trajectory, not
                      perfection.
                    </div>
                      ...
                      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                        ...
                        >
                          ...
                          <div style={{ opacity: 0.75, fontSize: 12 }}>
                            Generated:{" "}
                            {scorecard.ts.replace("T", " ").slice(0, 19)}Z
                          </div>
                        </div>
                        ...
                        {scorecardDeltaSummary ? (
                          ...
                          >
                            ...
                            <span style={{ fontSize: 12, opacity: 0.9 }}>
                              vs {previousScorecard?.monthKey}
                            </span>
                            <span style={{ fontSize: 12, opacity: 0.9 }}>
                              Improving:{" "}
                              <strong>{scorecardDeltaSummary.improving}</strong>
                            </span>
                            <span style={{ fontSize: 12, opacity: 0.9 }}>
                              Flat:{" "}
                              <strong>{scorecardDeltaSummary.flat}</strong>
                            </span>
                            <span style={{ fontSize: 12, opacity: 0.9 }}>
                              Down:{" "}
                              <strong>{scorecardDeltaSummary.down}</strong>
                            </span>
                          </div>
                        ) : null}
                        ...
                        <div
                          style={{
                            display: "grid",
                            gridTemplateColumns: scorecardDeltaSummary
                              ? "1fr auto auto"
                              : "1fr auto",
                            gap: 6,
                            alignItems: "center",
                        ...
                        >
                          {scorecardMetrics.map((metric) => {
                            ...
                            const delta =
                              scorecardDeltaSummary?.deltas.find(
                                (d) => d.key === metric.key,
                              )?.delta ?? null;
                            return (
                        ...
                        <div style={{ opacity: 0.9 }}>
                          Momentum:{" "}
                          <strong>
                            {scorecard.momentum === "up"
                              ? "↑ Improving"
                              : scorecard.momentum === "down"
                                ? "↓ Slipping"
                                : "→ Flat"}
                          </strong>
                          {previousScorecard ? (
                            <span style={{ opacity: 0.8 }}>
                              ...
                              • Previous:{" "}
                              <strong>
                                {previousScorecard.momentum === "up"
                                  ? "↑ Improving"
                                  : previousScorecard.momentum === "down"
                                    ? "↓ Slipping"
                                    : "→ Flat"}
                              </strong>
                            </span>
                    ...
                    {scoreShowHistory && scoreHistory.length > 0 ? (
                      ...
                      >
                        ...
                        >
                          Previous scorecards (click to load):
                        </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ fontSize: 12, opacity: 0.75 }}>
                        {aiInsightHistory[0]?.ts
                          ? `Last run: ${aiInsightHistory[0].ts.replace("T", " ").slice(0, 19)}Z`
                          : ""}
                      </div>
                    </div>
                    <pre
                    ...
                    {aiShowHistory && aiInsightHistory.length > 0 ? (
                      ...
                      >
                        ...
                        >
                          Previous coach runs (click to load):
                        </div>
                        <div style={{ display: "grid", gap: 6 }}>
                ...
                {visionText ? (
                  <div style={{ marginTop: 12 }}>
                    ...
                    >
                      ...
                      {visionPose.toUpperCase()} (
                      {visionScope === "month" ? "month" : "last 2"},{" "}
                      {visionFocus})
                    </div>
                    <pre
                      style={{
                    ...
                    {visionShowHistory && visionHistory.length > 0 ? (
                      ...
                      >
                        ...
                        >
                          Previous Vision runs (click to load):
                        </div>
                        <div style={{ display: "grid", gap: 6 }}>
                          {visionHistory.map((h) => (
                            <button
                  ...
                  <div>
                    ...
                    >
                      <label>
                        ...
                        <select
                          value={flipPose}
                          onChange={(e) => setFlipPose(e.target.value as Pose)}
                          style={{ padding: 6 }}
                        >
                          <option value="front">Front</option>
                          <option value="quarter">Quarter Turn</option>
                      ...
                      <label>
                        ...
                        <select
                          value={flipView}
                          onChange={(e) => setFlipView(e.target.value as any)}
                          style={{ padding: 6 }}
                        >
                      ...
                      {flipView !== "normal" ? (
                        ...
                        >
                          ...
                          <input
                            ...
                            value={ghostOpacity}
                            onChange={(e) =>
                              setGhostOpacity(Number(e.target.value))
                            }
                            style={{ width: 140 }}
                          />
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: 6, display: "grid", gap: 4 }}>
                      {CORE_POSES.map((p) => {
                        ...
                        return (
                          <div key={p}>
                            {p.toUpperCase()}: {h.first.taken_on} →{" "}
                            {h.last.taken_on}
                          </div>
                        );
                      })}
                  ...
                  <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
                    ...
                    >
                      <div style={{ opacity: 0.9 }}>
                        ...
                        {flipList[flipIdx]
                          ? `${flipList[flipIdx].taken_on} (${flipPose.toUpperCase()})`
                          : ""}
                      </div>
                      <div style={{ opacity: 0.75, fontSize: 12 }}>
                        Frame {flipList.length ? flipIdx + 1 : 0} /{" "}
                        {flipList.length}
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <span>
                        {flipList[flipList.length - 1]?.taken_on ?? ""}
                      </span>
                    </div>
                  </div>
                ) : null}
                ...
                {flipList[flipIdx] && thumbs[flipList[flipIdx].id] ? (
                  <div style={{ marginTop: 10 }}>
                    ...
                    >
                      ...
                      <img
                        ...
                        style={{
                          ...
                          display: "block",
                          objectFit: "contain",
                          transform: `translate(${alignX}px, ${alignY}px)`,
                        }}
                      />
                    ...
                    >
                      ...
                      >
                        <button
                          onClick={(e) => nudgeAlign(0, e.shiftKey ? -10 : -2)}
                        ...
                        >
                          <button
                            onClick={(e) =>
                              nudgeAlign(e.shiftKey ? -10 : -2, 0)
                            }
                            title="Nudge left (Shift = 10px)"
                          >
                            ←
                          </button>
                          <button onClick={resetAlign} title="Reset alignment">
                            Reset
                          </button>
                          <button
                            onClick={(e) => nudgeAlign(e.shiftKey ? 10 : 2, 0)}
                      ...
                      <div style={{ display: "grid", gap: 6 }}>
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={alignGrid}
                            onChange={(e) => setAlignGrid(e.target.checked)}
                          />
                          Show grid
                        </label>
                        <label
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={flipKeysArmed}
                            onChange={(e) => setFlipKeysArmed(e.target.checked)}
                          />
                          Keyboard nudges
                        </label>
                      </div>
                      ...
                      {flipIdx > 0 ? (
                        <button
                          onClick={copyPrevAlignToCurrent}
                          title="Copy previous frame alignment to this frame"
                        >
                          Copy prev alignment
                        </button>
                      ) : (
                        <button
                          disabled
                          title="Log another week to copy alignment"
                        >
                          Copy prev alignment
                        </button>
                      )}
                      ...
                      <div style={{ opacity: 0.8, fontSize: 12 }}>
                        {flipKeysArmed
                          ? "Keys: ← ↑ ↓ → (Shift=10px), R=reset"
                          : "Enable keyboard nudges for arrow keys"}
                        <div>
                          Current offset: <strong>{alignX}</strong>,{" "}
                          <strong>{alignY}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    {flipView !== "normal" && flipIdx > 0 ? (
                      <div
                        style={{ marginTop: 8, opacity: 0.85, fontSize: 12 }}
                      >
                        {flipView === "ghost"
                          ? "Ghost"
                          : flipView === "diff"
                            ? "Heatmap"
                            : "Change map"}
                        : {flipList[flipIdx - 1].taken_on} →{" "}
                        {flipList[flipIdx].taken_on}
                      </div>
                    ) : null}
                    ...
                    {flipList.length < 2 ? (
                      <div
                        style={{ marginTop: 8, opacity: 0.85, fontSize: 12 }}
                      >
                        Flipbook needs <strong>2+</strong> anchor weeks for this
                        pose. Log next week’s {flipPose.toUpperCase()} to unlock
                        playback.
                      </div>
                    ) : null}
                  </div>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      {r.is_anchor ? (
                        <span style={{ marginLeft: 8 }}>(Anchor)</span>
                      ) : null}
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {r.is_anchor && CORE_POSES.includes(r.pose) ? (
                        <button onClick={() => openCompareForRow(r)}>
                          Compare
                        </button>
                      ) : null}
            ...
            {compareOpen && compareA && compareB ? (
              ...
              >
                ...
                >
                  ...
                  >
                    <div>
                      <strong>Compare</strong> — {compareB.pose.toUpperCase()} (
                      {compareA.taken_on} → {compareB.taken_on})
                    </div>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      <label style={{ fontSize: 12, opacity: 0.9 }}>
                        ...
                        <select
                          value={compareView}
                          onChange={(e) =>
                            setCompareView(e.target.value as any)
                          }
                          style={{ padding: 6 }}
                        >
                  ...
                  <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
                            ...
                            <img
                              ...
                              onPointerDown={(e) => {
                                ...
                                compareDragRef.current = {
                                  ...
                                  sx: e.clientX,
                                  sy: e.clientY,
                                  ax: (((compareB.align_x ?? 0) as number) ||
                                    0) as number,
                                  ay: (((compareB.align_y ?? 0) as number) ||
                                    0) as number,
                                };
                              }}
                              onPointerMove={(e) => {
                              ...
                              onPointerUp={() => {
                                if (compareDragRef.current)
                                  compareDragRef.current.active = false;
                              }}
                              onPointerCancel={() => {
                                if (compareDragRef.current)
                                  compareDragRef.current.active = false;
                    ...
                    >
                      <div style={{ fontSize: 12, opacity: 0.85 }}>
                        <b>Align:</b> drag the top photo, or nudge with buttons
                        (double‑click = bigger). “Reset” zeros alignment for the
                        top photo.
                      </div>
                      <div
                        style={{
                      ...
                      >
                        <button
                          className="btn"
                          onClick={() => compareNudge(0, -2)}
                          onDoubleClick={() => compareNudge(0, -10)}
                          title="Up"
                        >
                          ↑
                        </button>
                        <button
                          className="btn"
                          onClick={() => compareNudge(-2, 0)}
                          onDoubleClick={() => compareNudge(-10, 0)}
                          title="Left"
                        >
                          ←
                        </button>
                    ...
                    >
                      ...
                      <input
                        ...
                        max={100}
                        value={
                          compareView === "slider" ? compareMix : compareOpacity
                        }
                        onChange={(e) =>
                          compareView === "slider"
                            ? setCompareMix(Number(e.target.value))
      ...
      {mode === "measures" && (
        <div>
          <h3>Measurements</h3>
          <p style={{ opacity: 0.8 }}>
            One set per day. Auto-fills weight/waist from Quick Log when
            available.
          </p>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setMRow((p) => ({
                    ...p,
                    waist_in: e.target.value ? Number(e.target.value) : null,
                  }))
                }
                style={{ width: 120 }}
              />
